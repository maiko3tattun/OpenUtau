...
namespace OpenUtau.App.Views {
    ...
    class PhonemeChangeAttackTimeState : NoteEditState {
        ...
        public override void Update(IPointer pointer, Point point) {
            ...
            double attackTimeDelta = project.timeAxis.TickPosToMsPos(notesVm.PointToTick(point) + partPos) - p1x; // min max
            if (notesVm.Part == null) {
                return;
            }
            DocManager.Inst.ExecuteCmd(new PhonemeAttackTimeCommand(notesVm.Part, leadingNote, index, phoneme, (float)attackTimeDelta));
            valueTip.UpdateValueTip($"{attackTimeDelta:+0.0;-0.0;0}ms");
        }
    }
    ...
    class PhonemeChangeReleaseTimeState : NoteEditState {
        ...
        public override void Update(IPointer pointer, Point point) {
            ...
            var project = notesVm.Project;
            int partPos = notesVm.Part?.position ?? 0;
            float p2x = phoneme.envelope.data[2].X;
            float p4x = phoneme.envelope.data[4].X;
            double p3x = phoneme.PositionMs + Math.Max(p2x, p4x - phoneme.GetFadeOut());
            double releaseTimeDelta = p3x - project.timeAxis.TickPosToMsPos(notesVm.PointToTick(point) + partPos); // min max
            if (notesVm.Part == null) {
                return;
            }
            DocManager.Inst.ExecuteCmd(new PhonemeReleaseTimeCommand(notesVm.Part, leadingNote, index, phoneme, (float)releaseTimeDelta));
            valueTip.UpdateValueTip($"{releaseTimeDelta:+0.0;-0.0;0}ms");
        }
    }
